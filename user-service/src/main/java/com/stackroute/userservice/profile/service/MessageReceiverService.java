...
public class MessageReceiverService {
    ...
    @KafkaListener(topics = "orders", groupId = "group_json",
            containerFactory = "kafkaListenerContainerFactory")
    public void receive(OrderDto message) {
        try {
            logger.info("Consumed Message: " + message);
            logger.info("Latch Value: " + getLatch());
            Order order = new Order();
            order.setCustomerId(order.getCustomerId());
            order.setBillingDate(message.getBillingDate());
            order.setOrderId(message.getOrderId());
            order.setPaymentDetails(message.getPaymentDetails());
            order.setStatus(message.getStatus());
            order.setTotalBillAmount(message.getTotalBillAmount());
